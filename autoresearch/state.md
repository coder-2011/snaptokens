...
The `70x` target is not yet reached. Remaining measured costs are the Viterbi recurrence itself (all cheap lanes closed) and asymmetric P/E-core straggle on this host; on the 4-vCPU GCP basis, full parallelization bounds the reachable wall near `44x`–`50x` without a single-thread Viterbi reduction. Cross-host confirmation on the GCP diagnostic host is required before updating any recorded ratio; the local per-document paired-median protocol (seven counterbalanced cycles on the pinned 20-input fixture) is the screening basis for every verdict above. The `fuzz_unigram` independent-reference target and the T5 integration tests remain the required low-cost exactness gates.
