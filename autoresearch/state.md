...
### Scoped Unigram T5 encoding campaign (closed pending a new profile-backed mechanism)

The retained Hugging Face JSON Unigram implementation remains a T5-specialist result. Its strongest recorded full pinned-fixture run is `26.46x` Snaptokens/Hugging Face; that is end-to-end T5 encoding evidence only, not the separate GPT-2 JSON-load `71.69x` BPE claim and not a general promotion. The current `perf/unigram-json` source is restored to the retained Viterbi/matcher behavior; its branch history contains rejected candidates and durable evidence only.

The source-free matcher screens and source candidates have now exceeded the eight-consecutive-rejection stop rule. Rejected lanes include manual/direct double-array traversal, a replacement iterator, local end-group winner ownership, tie-condition removal, UTF-8 boundary specialization, direct known-path backtracking, score bounds, and two predecessor-packing layouts. Each completed exact Hugging Face IDs and/or focused test/fuzz gates as recorded in `log.md`; recent target screens ranged from `0.931367x` to `0.972925x` candidate/parent throughput. The fresh GCP profile still places about two thirds of this workload in Viterbi, but it does not identify another removable cost beyond the rejected matcher/state/output-chain work.

Do not make another Unigram performance source edit, tune a model/input-specific dispatch, or claim it matches BPE speed. Resume only from a new attributable profile that supports a materially different representation or algorithm, with a fresh experiment card. The existing `fuzz_unigram` independent-reference target and T5 scalar/repeated-prefix integration tests remain the required low-cost exactness gates. The GCP `simple_bench` probe is diagnostic T5-specialist evidence, not the frozen general evaluator.
