...
root=Path.home()/'st-campaign-20260917';data=root/'st-transfer-panel-20260917';out=root/'evidence/e13-pmu';out.mkdir()
binaries={'parent':root/'combined-profile-st-eval','candidate':root/'e13-candidate-st-eval'}
models=sorted(p for p in data.glob('*.json') if p.name!='corpus.json');env=dict(os.environ,RAYON_NUM_THREADS='4')
manifest={'purpose':'early owned file-buffer release mechanism only','pairs':3,'loads':100,'events':['cycles','instructions','branch-misses','minor-faults'],'worker_env':'explicit sudo forwarding','binaries':{k:hashlib.sha256(p.read_bytes()).hexdigest() for k,p in binaries.items()},'inputs':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [*models,data/'corpus.json']}}
