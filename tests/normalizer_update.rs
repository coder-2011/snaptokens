...
fn replacement_matches_fresh_construction() {
    ...
    for next in [
        ...
        for input in inputs {
            ...
            assert_eq!(ids, fresh.encode(input).unwrap(), "{input:?}");
            assert_eq!(
                ids,
        ...
        assert_eq!(tokenizer.encode("e\u{301}?").unwrap(), vec![7]);
    }
}
...
fn failed_replacement_preserves_consistent_state() {
    ...
    let before = tokenizer.encode("é!").unwrap();
    let invalid = json!({
        "type": "Replace", "pattern": { "String": "e\u{301}!" }, "content": ""
    });
    assert!(tokenizer.set_normalizer(normalizer(invalid)).is_err());
    assert_eq!(tokenizer.encode("é!").unwrap(), before);
    assert_eq!(tokenizer.encode("e\u{301}!").unwrap(), before);
}
