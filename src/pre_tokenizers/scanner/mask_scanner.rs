...
/// Skips ten direct-Han scalars at a time after a validated three-scalar prefix.
///
...
fn scan_kimi_han_run(bytes: &[u8], pos: usize) -> Option<usize> {
    ...
    #[cfg(target_arch = "x86_64")]
    if end + 41 <= bytes.len()
        && is_direct_kimi_han_start(bytes, end)
        && is_direct_kimi_han_start(bytes, end + 3)
        && is_direct_kimi_han_start(bytes, end + 6)
        && std::arch::is_x86_feature_detected!("avx2")
    {
        // SAFETY: the runtime guard proves AVX2; the three exact scalar predicates
        // establish the block's UTF-8 boundary and the helper checks its load bound.
        end = unsafe { skip_direct_kimi_han_blocks_avx2(bytes, end + 9) };
    }
