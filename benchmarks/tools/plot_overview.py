...
def main():
    ...
    source = (
        root
        / "benchmarks/data/2026-07-31/portable-current/summaries/encode-medians.csv"
    )
    ...
    baseline = {
        (r["host"], r["model"], r["shape"]): float(r["mib_per_s"])
        for r in rows
        if r["implementation"] == "huggingface-json" and r["contract"] == "nested"
    }
    series = []
    for engine, contract, label, color in [
        ...
        selected = [
            r
            for r in rows
            if r["implementation"] == engine and r["contract"] == contract
        ]
        ...
        if (
            ...
            or len(selected) != 780
            or actual != expected
            or len(actual) != len(selected)
            or actual != baseline.keys()
        ):
            raise ValueError(
                f"Expected the complete historical {contract} comparison for {engine}"
            )
        # Divide actual recorded medians, not ratios with different Snaptokens denominators.
        values = [
            geometric_mean(
                float(r["mib_per_s"]) / baseline[(r["host"], r["model"], shape)]
                for r in selected
                if r["shape"] == shape
            )
            for shape in shapes
    ...
    plt.rcParams.update(
        {
            ...
            "svg.fonttype": "path",
            "svg.hashsalt": "snaptokens-july-2026",
        }
    )
    background, ink, secondary = "#faf9f6", "#242424", "#65645f"
    fig, ax = plt.subplots(figsize=(14.4, 7.0), facecolor=background)
    fig.subplots_adjust(left=0.07, right=0.97, top=0.78, bottom=0.19)
    fig.text(
        0.045,
        0.91,
        "BPE Tokenization throughput",
        fontsize=22,
        weight="medium",
        color=ink,
    )
    ...
    for index, (label, values, color) in enumerate(series):
        positions = [x + (index - 1) * 0.23 for x in range(5)]
        ax.bar(
            positions,
            values,
            width=0.20,
    ...
    for bar in list(ax.patches):
        ...
        ax.add_patch(
            FancyBboxPatch(
                ...
                mutation_aspect=x_scale / y_scale,
    ...
    ax.yaxis.grid(True, color="#dfded8", linewidth=0.8)
    ...
    fig.savefig(
        ...
        metadata={
            "Date": None,
            "Title": "BPE Tokenization throughput",
    ...
    pattern = (
        r'benchmark-throughput(?:-dark)?(?:-[a-f0-9]{12})?\.svg(?:\?v=[a-f0-9]+)?(?=")'
    )
